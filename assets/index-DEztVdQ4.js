...
`):pn(Math.max(0,.6-t*.12)),requestAnimationFrame(()=>{ne.scrollTop=ne.scrollHeight}),t>=n&&(window.clearInterval(a),G('<span class="green">Purring in binary.</span>'))},90)}function mn(e){const n=document.createElement("div");n.className="output-block command-echo";const t=document.createElement("span");t.className="prompt",t.innerHTML=`<span class="prompt-user">${je}</span><span class="prompt-symbol">$</span>`;const a=document.createElement("span");a.textContent=e,n.append(t,a),ne.appendChild(n)}function G(e,n=""){const t=`output-block ${n}`.trim();for(const o of Array.from(ne.children))o.className===t&&o.innerHTML===e&&o.remove();const a=document.createElement("div");a.className=t,a.innerHTML=e,ne.appendChild(a),requestAnimationFrame(()=>{a.scrollIntoView({block:"start"})})}function ze(e){const n=e.trim();if(!n)return;mn(n),le.at(-1)!==n&&le.push(n),be=le.length;const[t,...a]=n.split(/\s+/),o=t.toLowerCase(),i=a.join(" ").toLowerCase();if(o==="clear"){ne.replaceChildren(),G(Ne(),"welcome-block");return}if(["about","whoami","cat"].includes(o)){o!=="cat"||!i||i==="about.txt"?G(Wt()):i==="contact.vcf"?G(xe()):i.endsWith(".md")&&pe(i.slice(0,-3))?Te(i.slice(0,-3)):G(`<span class="red">cat: ${ge(i)}: No such file</span>`);return}const r={help:Ft,skills:Vt,experience:zt,projects:Yt,brew:Ut,lab:Jt,articles:Xt,utils:rn,contact:xe,games:ln,ls:dn,tree:sn,neofetch:cn};if(r[o]){G(r[o]());return}if(o==="cv"){st(),G('<span class="green">Downloading albertobarrago_cv.pdf…</span>');return}if(o==="history"){G(le.map((p,b)=>`<div><span class="muted">${String(b+1).padStart(3," ")}</span>  ${ge(p)}</div>`).join(""));return}if(o==="date"){G(new Intl.DateTimeFormat("it-IT",{dateStyle:"full",timeStyle:"long"}).format(new Date));return}if(o==="play"){fn(i)?(G(`<span class="green">Launching ${we[i]}…</span>`),yn(i)):G(`<span class="red">Unknown game${i?`: ${ge(i)}`:""}.</span> Try <button class="inline-command command" data-command="games">games</button>.`);return}if(o==="1337"){un();return}if(o==="sudo"&&i==="hire alberto"){G('<span class="green">Permission granted. Opening contact details…</span>'),G(xe());return}G(`<span class="red">command not found: ${ge(o)}</span><br>Type <button class="inline-command command" data-command="help">help</button> to see available commands.`)}function fn(e){return Object.hasOwn(He,e)}function gn(e){const n=qt[e].map(t=>`<button class="game-control-button" type="button" data-control-key="${t.key}">${t.label}</button>`).join("");return`<div class="game-fullscreen" id="game-overlay">
