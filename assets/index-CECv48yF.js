...
</article>`).join("")}`}function Ge(e){if(!e)return e;const n=new Date(`${e}T00:00:00`);return Number.isNaN(n.getTime())?e:new Intl.DateTimeFormat("it-IT",{day:"2-digit",month:"2-digit",year:"numeric"}).format(n)}function Jt(){return X.length===0?`<div class="output-title">Articles</div>
...
<span class="palette"><i></i><i></i><i></i><i></i><i></i><i></i></span></div></div>`}function cn(){return'<div class="ls-output"><button class="inline-command file" data-command="about">about.txt</button><button class="inline-command directory" data-command="skills">skills/</button><button class="inline-command file" data-command="experience">experience.log</button><button class="inline-command directory" data-command="projects">projects/</button><button class="inline-command directory" data-command="lab">lab/</button><button class="inline-command directory" data-command="articles">articles/</button><button class="inline-command directory" data-command="utils">utils/</button><button class="inline-command file" data-command="contact">contact.vcf</button><button class="inline-command directory" data-command="games">games/</button></div>'}function Ve(e,n){return e.map(t=>t.split("").map(a=>a===" "||Math.random()>=n?a:De[Math.floor(Math.random()*De.length)]).join("")).join(`
`)}function dn(e){return Ve(qe,e)}function pn(){const e=ne.querySelector(".ascii-banner");if(!e)return;const n=Le.join(`
`),t=9;let a=0;const o=window.setInterval(()=>{if(a+=1,a>=t){window.clearInterval(o),e.textContent=n;return}e.textContent=Ve(Le,Math.max(0,.55-a*.06))},70)}function hn(){const e=document.createElement("div");e.className="output-block ascii-banner accent",ne.appendChild(e);const n=6;let t=0;const a=window.setInterval(()=>{t+=1,e.textContent=t>=n?qe.join(`
`):dn(Math.max(0,.6-t*.12)),requestAnimationFrame(()=>{ne.scrollTop=ne.scrollHeight}),t>=n&&(window.clearInterval(a),N('<span class="green">Purring in binary.</span>'))},90)}function un(e){const n=document.createElement("div");n.className="output-block command-echo";const t=document.createElement("span");t.className="prompt",t.innerHTML=`<span class="prompt-user">${je}</span><span class="prompt-symbol">$</span>`;const a=document.createElement("span");a.textContent=e,n.append(t,a),ne.appendChild(n)}function N(e,n=""){const t=`output-block ${n}`.trim();for(const o of Array.from(ne.children))o.className===t&&o.innerHTML===e&&o.remove();const a=document.createElement("div");a.className=t,a.innerHTML=e,ne.appendChild(a),requestAnimationFrame(()=>{a.scrollIntoView({block:"start"})})}function ze(e){const n=e.trim();if(!n)return;un(n),le.at(-1)!==n&&le.push(n),be=le.length;const[t,...a]=n.split(/\s+/),o=t.toLowerCase(),i=a.join(" ").toLowerCase();if(o==="clear"){ne.replaceChildren(),N(Ne(),"welcome-block");return}if(["about","whoami","cat"].includes(o)){o!=="cat"||!i||i==="about.txt"?N(Wt()):i==="contact.vcf"?N(xe()):i.endsWith(".md")&&pe(i.slice(0,-3))?Te(i.slice(0,-3)):N(`<span class="red">cat: ${ge(i)}: No such file</span>`);return}const r={help:Ft,skills:Vt,experience:zt,projects:Yt,lab:Ut,articles:Jt,utils:an,contact:xe,games:rn,ls:cn,tree:ln,neofetch:sn};if(r[o]){N(r[o]());return}if(o==="cv"){st(),N('<span class="green">Downloading albertobarrago_cv.pdf…</span>');return}if(o==="history"){N(le.map((p,b)=>`<div><span class="muted">${String(b+1).padStart(3," ")}</span>  ${ge(p)}</div>`).join(""));return}if(o==="date"){N(new Intl.DateTimeFormat("it-IT",{dateStyle:"full",timeStyle:"long"}).format(new Date));return}if(o==="rss"){N(`<div class="output-title">RSS feed</div>
<p class="prose">Subscribe to my articles feed in any RSS reader:</p>
<div class="key-value"><span class="label">url</span><a class="terminal-link" href="/feed.xml" target="_blank" rel="noopener">https://albz.it/feed.xml</a></div>`);return}if(o==="play"){mn(i)?(N(`<span class="green">Launching ${we[i]}…</span>`),gn(i)):N(`<span class="red">Unknown game${i?`: ${ge(i)}`:""}.</span> Try <button class="inline-command command" data-command="games">games</button>.`);return}if(o==="1337"){hn();return}if(o==="sudo"&&i==="hire alberto"){N('<span class="green">Permission granted. Opening contact details…</span>'),N(xe());return}N(`<span class="red">command not found: ${ge(o)}</span><br>Type <button class="inline-command command" data-command="help">help</button> to see available commands.`)}function mn(e){return Object.hasOwn(He,e)}function fn(e){const n=qt[e].map(t=>`<button class="game-control-button" type="button" data-control-key="${t.key}">${t.label}</button>`).join("");return`<div class="game-fullscreen" id="game-overlay">
	...
	</div>`}function gn(e){Se(),B.blur();const n=document.createElement("div");n.innerHTML=fn(e);const t=n.firstElementChild;oe.appendChild(t);const a=t.querySelector(".game-canvas");requestAnimationFrame(()=>{t.isConnected&&(de=He[e](a,Se))})}function Se(){var e;de==null||de(),de=null,Ye(),(e=document.getElementById("game-overlay"))==null||e.remove(),B==null||B.focus({preventScroll:!0})}function ve(e,n){window.dispatchEvent(new KeyboardEvent(n,{key:e,bubbles:!0,cancelable:!0}))}function yn(e){var i;const t=e.target.closest("[data-control-key]"),a=t==null?void 0:t.dataset.controlKey;if(!t||!a)return;e.preventDefault(),(i=t.setPointerCapture)==null||i.call(t,e.pointerId);const o={key:a,button:t};he.set(e.pointerId,o),t.classList.add("is-pressed"),ve(a,"keydown"),Nt.has(a)&&(o.delayId=window.setTimeout(()=>{o.intervalId=window.setInterval(()=>ve(a,"keydown"),85)},220))}function Ce(e){const n=he.get(e.pointerId);n&&(e.preventDefault(),n.delayId&&window.clearTimeout(n.delayId),n.intervalId&&window.clearInterval(n.intervalId),n.button.classList.remove("is-pressed"),he.delete(e.pointerId),ve(n.key,"keyup"))}function Ye(){for(const e of he.values())e.delayId&&window.clearTimeout(e.delayId),e.intervalId&&window.clearInterval(e.intervalId),e.button.classList.remove("is-pressed"),ve(e.key,"keyup");he.clear()}oe.innerHTML=Gt();const ne=document.getElementById("terminal-output"),bn=document.getElementById("terminal-form"),B=document.getElementById("terminal-input");N(Ne(),"welcome-block");pn();St();function Ue(){var e,n;return((e=window.location.pathname.match(/^\/articles\/([^/]+)\/?$/))==null?void 0:e[1])??((n=window.location.hash.match(/^#article\/(.+)$/))==null?void 0:n[1])}const Ae=Ue();Ae&&pe(Ae)&&Te(Ae);window.addEventListener("popstate",()=>{var n;const e=Ue();e&&pe(e)?Te(e):(n=document.getElementById("article-reader"))==null||n.remove()});bn.addEventListener("submit",e=>{e.preventDefault();const n=B.value;B.value="",ze(n)});B.addEventListener("input",()=>{te=null});B.addEventListener("keydown",e=>{if(e.key==="ArrowUp"||e.key==="ArrowDown"){e.preventDefault();const n=e.key==="ArrowUp"?-1:1;be=Math.max(0,Math.min(le.length,be+n)),B.value=le[be]??"",requestAnimationFrame(()=>B.setSelectionRange(B.value.length,B.value.length));return}if(e.key==="Tab"){e.preventDefault();const n=B.value.trim().toLowerCase();if(!n){N(`<span class="muted">Type 'help' to see the list of available commands.</span>`),te=null;return}const t=jt.filter(o=>o.startsWith(n));if(t.length===0){te=null;return}if(t.length===1){B.value=t[0],te=null;return}const a=t.reduce((o,i)=>{let r=0;for(;r<o.length&&r<i.length&&o[r]===i[r];)r+=1;return o.slice(0,r)});if(a.length>n.length){B.value=a,te=null;return}te===n?(N(t.map(o=>`<span class="command">${o}</span>`).join("  ")),te=null):te=n}});oe.addEventListener("click",e=>{var b,A,v;const n=e.target,t=document.querySelector("#reader-toc.is-open");t&&!n.closest("#reader-toc")&&!n.closest('[data-action="toggle-toc"]')&&t.classList.remove("is-open");const a=n.closest('[data-action="read-article"]');if(a!=null&&a.dataset.slug){Te(a.dataset.slug);return}if(n.closest('[data-action="close-reader"]')){Fe();return}const o=n.closest('[data-action="share-article"]');if(o!=null&&o.dataset.slug){on(o.dataset.slug,o);return}const i=n.closest('[data-action="copy-article-link"]');if(i!=null&&i.dataset.slug){We(i.dataset.slug,i);return}if(n.closest('[data-action="toggle-toc"]')){(b=document.getElementById("reader-toc"))==null||b.classList.toggle("is-open");return}const r=n.closest('[data-action="goto-heading"]');if(r!=null&&r.dataset.target){const S=document.getElementById("article-reader");(A=document.getElementById(r.dataset.target))==null||A.scrollIntoView({behavior:"smooth",block:"start"}),(v=S==null?void 0:S.querySelector("#reader-toc"))==null||v.classList.remove("is-open");return}const p=n.closest("[data-command]");p!=null&&p.dataset.command&&ze(p.dataset.command),n.closest('[data-action="exit-game"]')&&Se(),!n.closest("a")&&!document.getElementById("game-overlay")&&!document.getElementById("article-reader")&&B.focus({preventScroll:!0})});oe.addEventListener("pointerdown",yn);oe.addEventListener("pointerup",Ce);oe.addEventListener("pointercancel",Ce);oe.addEventListener("lostpointercapture",Ce);window.addEventListener("blur",Ye);document.addEventListener("keydown",e=>{e.key==="Escape"&&document.getElementById("game-overlay")&&Se(),e.key==="Escape"&&document.getElementById("article-reader")&&Fe()});
