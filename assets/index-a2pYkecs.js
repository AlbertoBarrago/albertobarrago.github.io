...
technologies.
`,fe={frontend:["Angular","React","Vanilla JS"],backend:["Node.js","Fastify","FastAPI","Flask","Spring Boot"],database:["MongoDB","Oracle","MySQL"],devops:["Docker","GitHub/Gitlab Actions","Azure DevOps"],tools:["Git","Neovim","tmux","eza","Wir"],ai:["LLM Integration","Prompt Engineering","AI Agents"]},de=[{role:"Senior Software Engineer",company:"C22",period:"2026 - Present",highlight:"Fetching..."},{role:"Senior Software Developer",company:"Softfobia",period:"2022 - 2023",highlight:"Led full-stack teams, 20% load time improvement"},{role:"Software Developer",company:"Accenture",period:"2021 - 2022",highlight:"Kafka microservices architecture, Angular 12 apps"},{role:"Software Developer",company:"Sinossi",period:"2016 - 2021",highlight:"Mobile apps for Deutsche Bank, TIM (Fortune 500)"}],W={github:"https://github.com/AlbertoBarrago",email:`mailto:${ae}`,blog:"https://alblog.gigalixirapp.com/articles"},Y=ne.version,pe=()=>{const t=document.createElement("a");t.href=j,t.download=j,t.dispatchEvent(new MouseEvent("click"))};function he(t,r){const e=t.getContext("2d");if(!e)return()=>{};const n=8,s=5,a=32,h=24,p=12,T=40,L=20,y=3,_=12,x=5,l=7,P=4,O=.003,v=["#ff6b6b","#ff6b6b","#ffbd2e","#ffbd2e","#00ff41"];let c="waiting",w=0,m=3,d=parseInt(localStorage.getItem("spaceInvadersHigh")||"0",10),k=0;function V(){t.width=t.clientWidth,t.height=t.clientHeight}V();const D=new ResizeObserver(V);D.observe(t);let R=t.width/2-T/2;const M=()=>t.height-40;let I=[],H=1,N=0,b=40;const o=10,f=20;function C(){I=[];const i=n*(a+p)-p,E=(t.width-i)/2;for(let B=0;B<s;B++)for(let F=0;F<n;F++)I.push({x:E+F*(a+p),y:60+B*(h+p),row:B,alive:!0});H=1,N=0,b=40}let g=[],u=[],S=0;const A={};function G(i){if(i.key==="Escape"){r();return}(c==="waiting"||c==="gameover"||c==="won")&&i.key==="Enter"&&(c="playing",w=0,m=3,g=[],u=[],R=t.width/2-T/2,C()),A[i.key]=!0,(i.key===" "||i.key==="ArrowLeft"||i.key==="ArrowRight")&&i.preventDefault()}function $(i){A[i.key]=!1}window.addEventListener("keydown",G),window.addEventListener("keyup",$);function ie(){if(c==="playing"){A.ArrowLeft&&R>0&&(R-=x),A.ArrowRight&&R<t.width-T&&(R+=x),S>0&&S--,A[" "]&&S===0&&(g.push({x:R+T/2-y/2,y:M()-_}),S=15);for(let i=g.length-1;i>=0;i--){if(g[i].y-=l,g[i].y<0){g.splice(i,1);continue}for(const E of I)if(E.alive&&g[i]&&g[i].x<E.x+a&&g[i].x+y>E.x&&g[i].y<E.y+h&&g[i].y+_>E.y){E.alive=!1,g.splice(i,1),w+=10,w>d&&(d=w,localStorage.setItem("spaceInvadersHigh",String(d)));const B=I.filter(F=>F.alive).length;B>0&&(b=Math.max(4,Math.floor(40*(B/(s*n)))));break}}if(I.every(i=>!i.alive)){c="won";return}if(N++,N>=b){N=0;let i=!1;for(const E of I)if(E.alive&&(H>0&&E.x+a+o>t.width-10||H<0&&E.x-o<10)){i=!0;break}if(i){H*=-1;for(const E of I)E.y+=f}else for(const E of I)E.x+=o*H;for(const E of I)if(E.alive&&E.y+h>=M()){c="gameover";return}}for(const i of I.filter(E=>E.alive))Math.random()<O&&u.push({x:i.x+a/2-y/2,y:i.y+h});for(let i=u.length-1;i>=0;i--){if(u[i].y+=P,u[i].y>t.height){u.splice(i,1);continue}if(u[i].x<R+T&&u[i].x+y>R&&u[i].y<M()+L&&u[i].y+_>M()&&(u.splice(i,1),m--,m<=0)){c="gameover";return}}}}function oe(){if(e){if(e.fillStyle="#0a0a0a",e.fillRect(0,0,t.width,t.height),c==="waiting"){e.fillStyle="#00ff41",e.textAlign="center",e.font="48px VT323, monospace",e.fillText("SPACE INVADERS",t.width/2,t.height/2-60),e.font="24px VT323, monospace",e.fillStyle="#ffbd2e",e.fillText("PRESS ENTER TO START",t.width/2,t.height/2+10),e.fillStyle="#888",e.font="18px VT323, monospace",e.fillText("Arrow Keys = Move | Space = Shoot | ESC = Exit",t.width/2,t.height/2+50),e.fillText("High Score: "+d,t.width/2,t.height/2+80),e.textAlign="left";return}e.fillStyle="#00bfff",e.beginPath(),e.moveTo(R+T/2,M()),e.lineTo(R,M()+L),e.lineTo(R+T,M()+L),e.closePath(),e.fill();for(const i of I)i.alive&&(e.fillStyle=v[i.row],e.fillRect(i.x+4,i.y,a-8,h-4),e.fillRect(i.x+2,i.y+4,6,4),e.fillRect(i.x+a-8,i.y+4,6,4),e.fillRect(i.x+6,i.y-4,3,6),e.fillRect(i.x+a-9,i.y-4,3,6),e.fillRect(i.x,i.y+h-6,4,6),e.fillRect(i.x+a-4,i.y+h-6,4,6));e.fillStyle="#00ff41";for(const i of g)e.fillRect(i.x,i.y,y,_);e.fillStyle="#ff6b6b";for(const i of u)e.fillRect(i.x,i.y,y,_);e.fillStyle="#00ff41",e.font="22px VT323, monospace",e.textAlign="left",e.fillText("SCORE: "+w,10,24),e.textAlign="center",e.fillText("HIGH: "+d,t.width/2,24),e.textAlign="right",e.fillText("LIVES: "+"♥".repeat(m),t.width-10,24),e.textAlign="left",c==="gameover"&&(e.fillStyle="rgba(0,0,0,0.7)",e.fillRect(0,0,t.width,t.height),e.fillStyle="#ff6b6b",e.textAlign="center",e.font="48px VT323, monospace",e.fillText("GAME OVER",t.width/2,t.height/2-20),e.fillStyle="#ffbd2e",e.font="24px VT323, monospace",e.fillText("Score: "+w,t.width/2,t.height/2+20),e.fillStyle="#888",e.fillText("PRESS ENTER TO RESTART",t.width/2,t.height/2+60),e.textAlign="left"),c==="won"&&(e.fillStyle="rgba(0,0,0,0.7)",e.fillRect(0,0,t.width,t.height),e.fillStyle="#00ff41",e.textAlign="center",e.font="48px VT323, monospace",e.fillText("YOU WIN!",t.width/2,t.height/2-20),e.fillStyle="#ffbd2e",e.font="24px VT323, monospace",e.fillText("Score: "+w,t.width/2,t.height/2+20),e.fillStyle="#888",e.fillText("PRESS ENTER TO PLAY AGAIN",t.width/2,t.height/2+60),e.textAlign="left")}}function z(){ie(),oe(),k=requestAnimationFrame(z)}return C(),z(),function(){cancelAnimationFrame(k),window.removeEventListener("keydown",G),window.removeEventListener("keyup",$),D.disconnect()}}function ge(t,r){const e=t.getContext("2d");if(!e)return()=>{};const n=10,s=20,a=[{shape:[[1,1,1,1]],color:"#00bfff"},{shape:[[1,1],[1,1]],color:"#ffbd2e"},{shape:[[0,1,0],[1,1,1]],color:"#a855f7"},{shape:[[1,0,0],[1,1,1]],color:"#ff6b6b"},{shape:[[0,0,1],[1,1,1]],color:"#00ff41"},{shape:[[0,1,1],[1,1,0]],color:"#ff6b6b"},{shape:[[1,1,0],[0,1,1]],color:"#00ff41"}];let h="waiting",p=0,T=1,L=0,y=parseInt(localStorage.getItem("tetrisHigh")||"0",10),_=0,x=[],l=null,P=0,O=45;function v(){t.width=t.clientWidth,t.height=t.clientHeight}v();const c=new ResizeObserver(v);c.observe(t);function w(){x=[];for(let o=0;o<s;o++)x.push(new Array(n).fill(null))}function m(){const o=a[Math.floor(Math.random()*a.length)];l={shape:o.shape.map(f=>[...f]),color:o.color,x:Math.floor((n-o.shape[0].length)/2),y:0},d(l.shape,l.x,l.y)&&(h="gameover",l=null)}function d(o,f,C){for(let g=0;g<o.length;g++)for(let u=0;u<o[g].length;u++){if(!o[g][u])continue;const S=f+u,A=C+g;if(S<0||S>=n||A>=s||A>=0&&x[A][S])return!0}return!1}function k(){if(l){for(let o=0;o<l.shape.length;o++)for(let f=0;f<l.shape[o].length;f++){if(!l.shape[o][f])continue;const C=l.y+o;C>=0&&(x[C][l.x+f]=l.color)}V(),m()}}function V(){let o=0;for(let f=s-1;f>=0;f--)x[f].every(C=>C!==null)&&(x.splice(f,1),x.unshift(new Array(n).fill(null)),o++,f++);o>0&&(p+=([0,100,300,500,800][o]||800)*T,L+=o,T=Math.floor(L/10)+1,O=Math.max(5,45-(T-1)*4),p>y&&(y=p,localStorage.setItem("tetrisHigh",String(y))))}function D(o){const f=o.length,C=o[0].length,g=[];for(let u=0;u<C;u++){g.push([]);for(let S=f-1;S>=0;S--)g[u].push(o[S][u])}return g}const R={};function M(o){if(o.key==="Escape"){r();return}if((h==="waiting"||h==="gameover")&&o.key==="Enter"&&(h="playing",p=0,T=1,L=0,O=45,w(),m()),h!=="playing"||!l){R[o.key]=!0;return}if(o.key==="ArrowLeft")d(l.shape,l.x-1,l.y)||l.x--;else if(o.key==="ArrowRight")d(l.shape,l.x+1,l.y)||l.x++;else if(o.key==="ArrowDown")d(l.shape,l.x,l.y+1)?k():l.y++;else if(o.key==="ArrowUp"||o.key===" "){const f=D(l.shape);d(f,l.x,l.y)?d(f,l.x-1,l.y)?d(f,l.x+1,l.y)||(l.shape=f,l.x++):(l.shape=f,l.x--):l.shape=f}R[o.key]=!0,["ArrowLeft","ArrowRight","ArrowDown","ArrowUp"," "].includes(o.key)&&o.preventDefault()}function I(o){R[o.key]=!1}window.addEventListener("keydown",M),window.addEventListener("keyup",I);function H(){h!=="playing"||!l||(P++,P>=O&&(P=0,d(l.shape,l.x,l.y+1)?k():l.y++))}function N(){if(!e)return;e.fillStyle="#0a0a0a",e.fillRect(0,0,t.width,t.height);const o=Math.min(Math.floor((t.height-60)/s),Math.floor((t.width-200)/n)),f=o*n,C=o*s,g=Math.floor((t.width-f)/2),u=Math.floor((t.height-C)/2)+10;if(h==="waiting"){e.fillStyle="#00ff41",e.textAlign="center",e.font="48px VT323, monospace",e.fillText("TETRIS",t.width/2,t.height/2-60),e.font="24px VT323, monospace",e.fillStyle="#ffbd2e",e.fillText("PRESS ENTER TO START",t.width/2,t.height/2+10),e.fillStyle="#888",e.font="18px VT323, monospace",e.fillText("Arrow Keys = Move | Up/Space = Rotate | ESC = Exit",t.width/2,t.height/2+50),e.fillText("High Score: "+y,t.width/2,t.height/2+80),e.textAlign="left";return}e.strokeStyle="#333",e.lineWidth=2,e.strokeRect(g-1,u-1,f+2,C+2);for(let S=0;S<s;S++)for(let A=0;A<n;A++){const G=g+A*o,$=u+S*o;x[S][A]?(e.fillStyle=x[S][A],e.fillRect(G+1,$+1,o-2,o-2)):(e.fillStyle="#111",e.fillRect(G,$,o,o),e.strokeStyle="#1a1a1a",e.lineWidth=.5,e.strokeRect(G,$,o,o))}if(l){e.fillStyle=l.color;for(let S=0;S<l.shape.length;S++)for(let A=0;A<l.shape[S].length;A++){if(!l.shape[S][A])continue;const G=g+(l.x+A)*o,$=u+(l.y+S)*o;e.fillRect(G+1,$+1,o-2,o-2)}}e.fillStyle="#00ff41",e.font="22px VT323, monospace",e.textAlign="left",e.fillText("SCORE: "+p,10,24),e.fillText("LEVEL: "+T,10,50),e.textAlign="center",e.fillText("HIGH: "+y,t.width/2,24),e.textAlign="right",e.fillText("LINES: "+L,t.width-10,24),e.textAlign="left",h==="gameover"&&(e.fillStyle="rgba(0,0,0,0.7)",e.fillRect(0,0,t.width,t.height),e.fillStyle="#ff6b6b",e.textAlign="center",e.font="48px VT323, monospace",e.fillText("GAME OVER",t.width/2,t.height/2-20),e.fillStyle="#ffbd2e",e.font="24px VT323, monospace",e.fillText("Score: "+p+"  Lines: "+L,t.width/2,t.height/2+20),e.fillStyle="#888",e.fillText("PRESS ENTER TO RESTART",t.width/2,t.height/2+60),e.textAlign="left")}function b(){H(),N(),_=requestAnimationFrame(b)}return w(),b(),function(){cancelAnimationFrame(_),window.removeEventListener("keydown",M),window.removeEventListener("keyup",I),c.disconnect()}}function ue(t,r){const e=t.getContext("2d");if(!e)return()=>{};const n=12,s=80,a=10,h=5,p=4,T=3.5,L=7;let y="waiting",_=0,x=0,l=0;function P(){t.width=t.clientWidth,t.height=t.clientHeight}P();const O=new ResizeObserver(P);O.observe(t);let v=t.height/2-s/2,c=t.height/2-s/2,w=t.width/2,m=t.height/2,d=p,k=p*.5;function V(){w=t.width/2,m=t.height/2,d=p*(Math.random()>.5?1:-1),k=(Math.random()*2-1)*p*.5}const D={};function R(b){if(b.key==="Escape"){r();return}(y==="waiting"||y==="gameover")&&b.key==="Enter"&&(y="playing",_=0,x=0,v=t.height/2-s/2,c=t.height/2-s/2,V()),D[b.key]=!0,(b.key==="ArrowUp"||b.key==="ArrowDown")&&b.preventDefault()}function M(b){D[b.key]=!1}window.addEventListener("keydown",R),window.addEventListener("keyup",M);function I(){if(y!=="playing")return;D.ArrowUp&&v>0&&(v-=h),D.ArrowDown&&v<t.height-s&&(v+=h);const b=c+s/2;d>0?b<m-10?c+=T:b>m+10&&(c-=T):b<t.height/2-5?c+=T*.5:b>t.height/2+5&&(c-=T*.5),c=Math.max(0,Math.min(t.height-s,c)),w+=d,m+=k,m<=0&&(m=0,k=Math.abs(k)),m+a>=t.height&&(m=t.height-a,k=-Math.abs(k));const o=20;w<=o+n&&w+a>=o&&m+a>=v&&m<=v+s&&d<0&&(d=Math.abs(d)*1.05,k=((m+a/2-v)/s-.5)*p*2,w=o+n);const f=t.width-20-n;w+a>=f&&w<=f+n&&m+a>=c&&m<=c+s&&d>0&&(d=-Math.abs(d)*1.05,k=((m+a/2-c)/s-.5)*p*2,w=f-a),w<0&&(x++,x>=L?y="gameover":V()),w>t.width&&(_++,_>=L?y="gameover":V())}function H(){if(e){if(e.fillStyle="#0a0a0a",e.fillRect(0,0,t.width,t.height),y==="waiting"){e.fillStyle="#00ff41",e.textAlign="center",e.font="48px VT323, monospace",e.fillText("PONG",t.width/2,t.height/2-60),e.font="24px VT323, monospace",e.fillStyle="#ffbd2e",e.fillText("PRESS ENTER TO START",t.width/2,t.height/2+10),e.fillStyle="#888",e.font="18px VT323, monospace",e.fillText("Arrow Up/Down = Move | First to "+L+" wins | ESC = Exit",t.width/2,t.height/2+50),e.textAlign="left";return}if(e.setLineDash([8,8]),e.strokeStyle="#333",e.lineWidth=2,e.beginPath(),e.moveTo(t.width/2,0),e.lineTo(t.width/2,t.height),e.stroke(),e.setLineDash([]),e.fillStyle="#00ff41",e.fillRect(20,v,n,s),e.fillStyle="#ff6b6b",e.fillRect(t.width-20-n,c,n,s),e.fillStyle="#ffbd2e",e.fillRect(w,m,a,a),e.fillStyle="#00ff41",e.font="48px VT323, monospace",e.textAlign="center",e.fillText(String(_),t.width/2-60,55),e.fillStyle="#ff6b6b",e.fillText(String(x),t.width/2+60,55),e.textAlign="left",e.font="18px VT323, monospace",e.fillStyle="#888",e.textAlign="left",e.fillText("YOU",20,t.height-15),e.textAlign="right",e.fillText("CPU",t.width-20,t.height-15),e.textAlign="left",y==="gameover"){e.fillStyle="rgba(0,0,0,0.7)",e.fillRect(0,0,t.width,t.height);const b=_>=L;e.fillStyle=b?"#00ff41":"#ff6b6b",e.textAlign="center",e.font="48px VT323, monospace",e.fillText(b?"YOU WIN!":"CPU WINS",t.width/2,t.height/2-20),e.fillStyle="#ffbd2e",e.font="24px VT323, monospace",e.fillText(_+" - "+x,t.width/2,t.height/2+20),e.fillStyle="#888",e.fillText("PRESS ENTER TO RESTART",t.width/2,t.height/2+60),e.textAlign="left"}}}function N(){I(),H(),l=requestAnimationFrame(N)}return N(),function(){cancelAnimationFrame(l),window.removeEventListener("keydown",R),window.removeEventListener("keyup",M),O.disconnect()}}console.log(`%c
 ██████╗ ██████╗ ███████╗███╗   ██╗
██╔═══██╗██╔══██╗██╔════╝████╗  ██║
██║   ██║██████╔╝█████╗  ██╔██╗ ██║
██║   ██║██╔═══╝ ██╔══╝  ██║╚██╗██║
...
> $_ Hire me before the next sprint starts.`,"color: #ffbd2e; font-size: 13px; font-family: monospace;");const me=Object.freeze({space:"SPACE INVADERS",tetris:"TETRIS",pong:"PONG"}),ye=Object.freeze({space:he,tetris:ge,pong:ue}),K=Object.freeze(["Initializing modem...","ATZ OK","ATDT *67 555-0199","CONNECT 56000","","Amiga Workbench 3.1","Copyright (c) 1985-1994 Commodore-Amiga, Inc.","","Checking memory...","Chip RAM: 2048K","Fast RAM: 8192K","","Loading system...","Welcome, visitor!","","Type HELP for commands or click menu items.",""]),we=["about","skills","experience","contact"],be=`     _    __  __ ___ ____    _
    / \\  |  \\/  |_ _/ ___|  / \\
   / _ \\ | |\\/| || | |  _  / _ \\
  / ___ \\| |  | || | |_| |/ ___ \\
 /_/   \\_\\_|  |_|___\\____/_/   \\_\\`;let X=!1,J=!1,q=null;const U=document.getElementById("app");function Se(){return`<div class="start-overlay" data-action="start-boot">
	<div class="start-content">
		<div class="start-ascii"><pre>${be}</pre></div>
	...
	<div class="contact-info">
		...
		<p><span class="label">Blog:</span> <a href="${W.blog}" target="_blank">alblog.gigalixirapp.com</a></p>
