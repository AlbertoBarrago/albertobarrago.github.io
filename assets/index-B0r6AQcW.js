...
`,vt=Object.freeze(Object.defineProperty({__proto__:null,html:xt,meta:kt},Symbol.toStringTag,{value:"Module"})),Tt={title:"Otelma: building a local LLM runtime from scratch",date:"2026-08-31",tags:["go","llm","llama-cpp","apple-silicon","ai"]},At=`<p><strong>Otelma</strong> is a local LLM inference runtime I&#39;m building in Go.</p>
...
`,It=Object.freeze(Object.defineProperty({__proto__:null,html:At,meta:Tt},Symbol.toStringTag,{value:"Module"})),Et={title:"I Built a CLI to Answer One Question: What Will This Change Break?",date:"2026-08-27",tags:["programming","devops","go","opensource"]},Lt=`<p>We&#39;ve all been there.</p>
...
`,Mt=Object.freeze(Object.defineProperty({__proto__:null,html:Lt,meta:Et},Symbol.toStringTag,{value:"Module"})),_t={title:"wir: What Is Running, a Port and Process Inspector Written in C",date:"2025-12-30",tags:["c","systems-programming","cli","opensource"]},Rt=`<p>I recently released <strong>wir</strong> (What Is Running), a command-line tool written in C to inspect what&#39;s running on specific ports and get detailed process information. A project born from a practical need that turned into an opportunity to explore system programming in C.</p>
<h2>The Problem</h2>
...
`,Ct=Object.freeze(Object.defineProperty({__proto__:null,html:Rt,meta:_t},Symbol.toStringTag,{value:"Module"}));function Pt(e){return[...e].sort((n,t)=>n.date.localeCompare(t.date))}const Ot=Object.assign({"/src/content/articles/iron-doctrine.md":vt,"/src/content/articles/otelma.md":It,"/src/content/articles/serval-cli.md":Mt,"/src/content/articles/wir-what-is-running.md":Ct}),Ee=new Map,Z=Pt(Object.entries(Ot).map(([e,n])=>{const t=e.split("/").pop().replace(/\.md$/,"");return Ee.set(t,n.html),{slug:t,title:n.meta.title??t,date:n.meta.date??"",tags:n.meta.tags??[]}}));function pe(e){return Ee.has(e)?Ee.get(e):null}const He="alberto@portfolio:~",$t=Object.freeze(["help","about","skills","experience","projects","brew","lab","articles","utils","contact","cv","games","play","ls","tree","neofetch","history","date","clear"]),Dt=Object.freeze([["help","help"],["about","about"],["skills","skills"],["projects","projects"],["brew","brew"],["lab","lab"],["articles","articles"],["utils","utils"],["games","games"],["contact","contact"]]),we=Object.freeze({space:"SPACE INVADERS",tetris:"TETRIS",pong:"PONG",flappy:"FLAPPY BIRD"}),Bt=Object.freeze({space:[{label:"START",key:"Enter"},{label:"←",key:"ArrowLeft"},{label:"FIRE",key:" "},{label:"→",key:"ArrowRight"}],tetris:[{label:"START",key:"Enter"},{label:"←",key:"ArrowLeft"},{label:"ROT",key:"ArrowUp"},{label:"→",key:"ArrowRight"},{label:"↓",key:"ArrowDown"}],pong:[{label:"START",key:"Enter"},{label:"↑",key:"ArrowUp"},{label:"↓",key:"ArrowDown"}],flappy:[{label:"START",key:"Enter"},{label:"FLAP",key:" "}]}),je=Object.freeze({space:ct,tetris:gt,pong:yt,flappy:bt}),Ne=Object.freeze(["        /\\_/\\","       ( o.o )","        > ^ <","       /|   |\\","      (_|   |_)","         W W","","      I LOVE CAT"]),Le=Object.freeze(["    _    _     ____   _____","   / \\  | |   | __ ) |__  /","  / _ \\ | |   |  _ \\   / /"," / ___ \\| |___| |_) | / /_","/_/   \\_\\_____|____/ /____|"]),De="#%@*+=-<>/\\|",Ht=new Set(["ArrowLeft","ArrowRight","ArrowDown"]),me=new Map,le=[];let be=0,te=null,de=null,ee=null,J=null;const ie=document.getElementById("app");function jt(){const e=Dt.map(([n,t])=>`<button class="mobile-command" type="button" data-command="${t}">${n}</button>`).join("");return`<main class="terminal" aria-label="Alberto Barrago portfolio terminal">
...
<div class="skills-list">${Object.entries(nt).map(([e,n])=>`<div class="skill-row"><span class="label">${e}</span><span>${n.join("  ·  ")}</span></div>`).join("")}</div>`}function Ft(){return`<div class="output-title">Experience</div>
...
${ot.map(n=>`<article class="project-item lab-project">
...
</article>`).join("")}`}function qe(e){if(!e)return e;const n=new Date(`${e}T00:00:00`);return Number.isNaN(n.getTime())?e:new Intl.DateTimeFormat("it-IT",{day:"2-digit",month:"2-digit",year:"numeric"}).format(n)}function zt(){return Z.length===0?`<div class="output-title">Articles</div>
...
<div class="project-list">${[...Z].sort((n,t)=>n.date.localeCompare(t.date)).map(n=>`<article class="project-item">
	<div><button class="terminal-link project-name inline-command" data-command="cat ${n.slug}.md">${n.title} ↗</button><span class="project-language">${qe(n.date)}</span></div>
	<p class="muted">${n.tags.join("  ·  ")}</p>
		...
		<div class="article-reader-body">
			...
			<div class="article-reader-main">
				<article class="prose">
					<h1>${t.title}</h1>
					<p class="reader-meta muted">${qe(t.date)}${t.tags.length?` · ${t.tags.join(" · ")}`:""}</p>
...
<span class="palette"><i></i><i></i><i></i><i></i><i></i><i></i></span></div></div>`}function ln(){return'<div class="ls-output"><button class="inline-command file" data-command="about">about.txt</button><button class="inline-command directory" data-command="skills">skills/</button><button class="inline-command file" data-command="experience">experience.log</button><button class="inline-command directory" data-command="projects">projects/</button><button class="inline-command directory" data-command="brew">brew/</button><button class="inline-command directory" data-command="lab">lab/</button><button class="inline-command directory" data-command="articles">articles/</button><button class="inline-command directory" data-command="utils">utils/</button><button class="inline-command file" data-command="contact">contact.vcf</button><button class="inline-command directory" data-command="games">games/</button></div>'}function Ve(e,n){return e.map(t=>t.split("").map(a=>a===" "||Math.random()>=n?a:De[Math.floor(Math.random()*De.length)]).join("")).join(`
`)}function sn(e){return Ve(Ne,e)}function cn(){const e=ne.querySelector(".ascii-banner");if(!e)return;const n=Le.join(`
`),t=9;let a=0;const i=window.setInterval(()=>{if(a+=1,a>=t){window.clearInterval(i),e.textContent=n;return}e.textContent=Ve(Le,Math.max(0,.55-a*.06))},70)}function dn(){const e=document.createElement("div");e.className="output-block ascii-banner accent",ne.appendChild(e);const n=6;let t=0;const a=window.setInterval(()=>{t+=1,e.textContent=t>=n?Ne.join(`
`):sn(Math.max(0,.6-t*.12)),requestAnimationFrame(()=>{ne.scrollTop=ne.scrollHeight}),t>=n&&(window.clearInterval(a),q('<span class="green">Purring in binary.</span>'))},90)}function pn(e){const n=document.createElement("div");n.className="output-block command-echo";const t=document.createElement("span");t.className="prompt",t.innerHTML=`<span class="prompt-user">${He}</span><span class="prompt-symbol">$</span>`;const a=document.createElement("span");a.textContent=e,n.append(t,a),ne.appendChild(n)}function q(e,n=""){const t=`output-block ${n}`.trim();for(const i of Array.from(ne.children))i.className===t&&i.innerHTML===e&&i.remove();const a=document.createElement("div");a.className=t,a.innerHTML=e,ne.appendChild(a),requestAnimationFrame(()=>{a.scrollIntoView({block:"start"})})}function Ye(e){const n=e.trim();if(!n)return;pn(n),le.at(-1)!==n&&le.push(n),be=le.length;const[t,...a]=n.split(/\s+/),i=t.toLowerCase(),o=a.join(" ").toLowerCase();if(i==="clear"){ne.replaceChildren(),q(Ge(),"welcome-block");return}if(["about","whoami","cat"].includes(i)){i!=="cat"||!o||o==="about.txt"?q(Gt()):o==="contact.vcf"?q(Te()):o.endsWith(".md")&&pe(o.slice(0,-3))?ve(o.slice(0,-3)):q(`<span class="red">cat: ${ge(o)}: No such file</span>`);return}const r={help:Nt,skills:qt,experience:Ft,projects:Wt,brew:Vt,lab:Yt,articles:zt,utils:nn,contact:Te,games:on,ls:ln,tree:an,neofetch:rn};if(r[i]){q(r[i]());return}if(i==="cv"){st(),q('<span class="green">Downloading albertobarrago_cv.pdf…</span>');return}if(i==="history"){q(le.map((p,b)=>`<div><span class="muted">${String(b+1).padStart(3," ")}</span>  ${ge(p)}</div>`).join(""));return}if(i==="date"){q(new Intl.DateTimeFormat("it-IT",{dateStyle:"full",timeStyle:"long"}).format(new Date));return}if(i==="play"){mn(o)?(q(`<span class="green">Launching ${we[o]}…</span>`),un(o)):q(`<span class="red">Unknown game${o?`: ${ge(o)}`:""}.</span> Try <button class="inline-command command" data-command="games">games</button>.`);return}if(i==="1337"){dn();return}if(i==="sudo"&&o==="hire alberto"){q('<span class="green">Permission granted. Opening contact details…</span>'),q(Te());return}q(`<span class="red">command not found: ${ge(i)}</span><br>Type <button class="inline-command command" data-command="help">help</button> to see available commands.`)}function mn(e){return Object.hasOwn(je,e)}function hn(e){const n=Bt[e].map(t=>`<button class="game-control-button" type="button" data-control-key="${t.key}">${t.label}</button>`).join("");return`<div class="game-fullscreen" id="game-overlay">
	...
	</div>`}function un(e){Se(),B.blur();const n=document.createElement("div");n.innerHTML=hn(e);const t=n.firstElementChild;ie.appendChild(t);const a=t.querySelector(".game-canvas");requestAnimationFrame(()=>{t.isConnected&&(de=je[e](a,Se))})}function Se(){var e;de==null||de(),de=null,ze(),(e=document.getElementById("game-overlay"))==null||e.remove(),B==null||B.focus({preventScroll:!0})}function ke(e,n){window.dispatchEvent(new KeyboardEvent(n,{key:e,bubbles:!0,cancelable:!0}))}function fn(e){var o;const t=e.target.closest("[data-control-key]"),a=t==null?void 0:t.dataset.controlKey;if(!t||!a)return;e.preventDefault(),(o=t.setPointerCapture)==null||o.call(t,e.pointerId);const i={key:a,button:t};me.set(e.pointerId,i),t.classList.add("is-pressed"),ke(a,"keydown"),Ht.has(a)&&(i.delayId=window.setTimeout(()=>{i.intervalId=window.setInterval(()=>ke(a,"keydown"),85)},220))}function Ce(e){const n=me.get(e.pointerId);n&&(e.preventDefault(),n.delayId&&window.clearTimeout(n.delayId),n.intervalId&&window.clearInterval(n.intervalId),n.button.classList.remove("is-pressed"),me.delete(e.pointerId),ke(n.key,"keyup"))}function ze(){for(const e of me.values())e.delayId&&window.clearTimeout(e.delayId),e.intervalId&&window.clearInterval(e.intervalId),e.button.classList.remove("is-pressed"),ke(e.key,"keyup");me.clear()}ie.innerHTML=jt();const ne=document.getElementById("terminal-output"),gn=document.getElementById("terminal-form"),B=document.getElementById("terminal-input");q(Ge(),"welcome-block");cn();St();function Ue(){var e,n;return((e=window.location.pathname.match(/^\/articles\/([^/]+)\/?$/))==null?void 0:e[1])??((n=window.location.hash.match(/^#article\/(.+)$/))==null?void 0:n[1])}const Ae=Ue();Ae&&pe(Ae)&&ve(Ae);window.addEventListener("popstate",()=>{var n;const e=Ue();e&&pe(e)?ve(e):(n=document.getElementById("article-reader"))==null||n.remove()});gn.addEventListener("submit",e=>{e.preventDefault();const n=B.value;B.value="",Ye(n)});B.addEventListener("input",()=>{te=null});B.addEventListener("keydown",e=>{if(e.key==="ArrowUp"||e.key==="ArrowDown"){e.preventDefault();const n=e.key==="ArrowUp"?-1:1;be=Math.max(0,Math.min(le.length,be+n)),B.value=le[be]??"",requestAnimationFrame(()=>B.setSelectionRange(B.value.length,B.value.length));return}if(e.key==="Tab"){e.preventDefault();const n=B.value.trim().toLowerCase();if(!n){q(`<span class="muted">Type 'help' to see the list of available commands.</span>`),te=null;return}const t=$t.filter(i=>i.startsWith(n));if(t.length===0){te=null;return}if(t.length===1){B.value=t[0],te=null;return}const a=t.reduce((i,o)=>{let r=0;for(;r<i.length&&r<o.length&&i[r]===o[r];)r+=1;return i.slice(0,r)});if(a.length>n.length){B.value=a,te=null;return}te===n?(q(t.map(i=>`<span class="command">${i}</span>`).join("  ")),te=null):te=n}});ie.addEventListener("click",e=>{var b,A,k;const n=e.target,t=document.querySelector("#reader-toc.is-open");t&&!n.closest("#reader-toc")&&!n.closest('[data-action="toggle-toc"]')&&t.classList.remove("is-open");const a=n.closest('[data-action="read-article"]');if(a!=null&&a.dataset.slug){ve(a.dataset.slug);return}if(n.closest('[data-action="close-reader"]')){Fe();return}const i=n.closest('[data-action="share-article"]');if(i!=null&&i.dataset.slug){tn(i.dataset.slug,i);return}const o=n.closest('[data-action="copy-article-link"]');if(o!=null&&o.dataset.slug){We(o.dataset.slug,o);return}if(n.closest('[data-action="toggle-toc"]')){(b=document.getElementById("reader-toc"))==null||b.classList.toggle("is-open");return}const r=n.closest('[data-action="goto-heading"]');if(r!=null&&r.dataset.target){const S=document.getElementById("article-reader");(A=document.getElementById(r.dataset.target))==null||A.scrollIntoView({behavior:"smooth",block:"start"}),(k=S==null?void 0:S.querySelector("#reader-toc"))==null||k.classList.remove("is-open");return}const p=n.closest("[data-command]");p!=null&&p.dataset.command&&Ye(p.dataset.command),n.closest('[data-action="exit-game"]')&&Se(),!n.closest("a")&&!document.getElementById("game-overlay")&&!document.getElementById("article-reader")&&B.focus({preventScroll:!0})});ie.addEventListener("pointerdown",fn);ie.addEventListener("pointerup",Ce);ie.addEventListener("pointercancel",Ce);ie.addEventListener("lostpointercapture",Ce);window.addEventListener("blur",ze);document.addEventListener("keydown",e=>{e.key==="Escape"&&document.getElementById("game-overlay")&&Se(),e.key==="Escape"&&document.getElementById("article-reader")&&Fe()});
